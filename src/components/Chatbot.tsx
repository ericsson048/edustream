...
export default function Chatbot() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      const payload = JSON.parse(event.data) as { kind?: string; response?: string; detail?: string };
      if (payload.kind === 'ai_response' && payload.response) {
        setMessages((prev) => [...prev, { id: `${Date.now()}-ai`, role: 'model', text: payload.response! }]);
        setIsLoading(false);
    ...
    socket.onerror = () => {
      showToast('Connexion temps reel IA indisponible.', 'error');
      setIsLoading(false);
    };
    return () => socket.close();
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      socketRef.current.send(JSON.stringify({ prompt: userMessage.text }));
    } catch (error) {
        ...
        <div className="mb-4 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[500px] animate-in slide-in-from-bottom-5 fade-in duration-200">
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-100">
            ...
            <div className="text-center mt-2">
              <p className="text-[10px] text-slate-400">Powered by realtime tutor</p>
            </div>
          </form>
        </div>
      )}
